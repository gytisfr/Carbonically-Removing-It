...
const TrucksPage = () => {
  ...
  const fetchTrucks = async () => {
    try {
      const res = await axios.get("http://127.0.0.1:5089/truck/fetch");
  ...
  const handleSubmit = async () => {
    if (!form.id || !form.routeid || !form.long || !form.lat) {
      return alert("Please fill in all required fields!");
    }

    try {
      await axios.post(
        "http://127.0.0.1:5089/truck",
        null,
        { params: form }
      );
      alert("Truck created successfully!");
      setForm({ id: "", routeid: "", long: "", lat: "", driverid: "", capacity: "" });
      fetchTrucks();
    } catch (error) {
      console.error(error);
  ...
  const handleEdit = async () => {
    ...
    try {
      for (const key of ["id", "routeid", "long", "lat", "driverid", "capacity"]) {
        if (selectedTruck[key] !== originalTruck[key]) {
          await axios.patch(
            "http://127.0.0.1:5089/truck",
            null,
            { params: { id: originalTruck.id, what: key, to: selectedTruck[key] } }
          );
        }
      }
      ...
      <main className="h-full w-full gap-6 grid grid-cols-3 grid-rows-1 items-start justify-center p-6">
          ...
          <div className="mt-6 space-y-4">
            ...
            <div>
              ...
              <input
                type="number"
                value={form.long}
                onChange={(e) => setForm(prev => ({ ...prev, long: e.target.value }))}
                className="border-black border-2 p-2 rounded-sm w-full"
              />
            </div>
        ...
        <div className="h-full w-full bg-white rounded-sm p-6 flex flex-col">
          ...
          <select
            className="border-2 border-black p-2 rounded mb-4"
            onChange={(e) => {
              const truck = trucks.find(t => t.id.toString() === e.target.value);
              if (!truck) return setSelectedTruck(null);
              ...
              <div className="space-y-2">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setSelectedTruck(prev => ({ ...prev, id: e.target.value }))}
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setSelectedTruck(prev => ({ ...prev, routeid: e.target.value }))}
                    className="border-2 border-black p-2 rounded w-full"
                  >
                    <option value="">-- Select Route --</option>
                    {routes.map(r => (
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setSelectedTruck(prev => ({ ...prev, long: e.target.value }))}
                    className="border-2 border-black p-2 rounded w-full"
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setSelectedTruck(prev => ({ ...prev, lat: e.target.value }))}
                    className="border-2 border-black p-2 rounded w-full"
                  />
                </div>
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setSelectedTruck(prev => ({ ...prev, driverid: e.target.value }))}
                    className="border-2 border-black p-2 rounded w-full"
                  >
                    <option value="">-- None --</option>
                    {drivers.map(d => (
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setSelectedTruck(prev => ({ ...prev, capacity: e.target.value }))}
                    className="border-2 border-black p-2 rounded w-full"
                  />
                </div>
              </div>
              ...
              <div className="flex gap-2 mt-4">
                ...
                <button
                  onClick={async () => {
                    ...
                    try {
                      await axios.delete("http://127.0.0.1:5089/truck", { params: { id: selectedTruck.id } });
